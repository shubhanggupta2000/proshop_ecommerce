  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `https://proshop-ecommerce-01vy.onrender.com/api/products?keyword=${keyword}&pageNumber=${pageNumber}`
      );
...
export const updateProduct = (product) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `https://proshop-ecommerce-01vy.onrender.com/api/products/${product._id}`,
      product,
      config
    );
